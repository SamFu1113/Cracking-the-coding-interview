...
A Google engineer conducts the first phone interview,so be prepared for tricky technical questions.这些问题可能涉及编码，有时需要通过共享文档进行编码。应聘者通常被要求遵守相同的标准，在电话视频面试（phone screens）上被问到的问题与现场面试（on-site）时类似。
